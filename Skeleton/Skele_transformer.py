...
class class_position_embedding(nn.Module):
    def __init__(self, d_model, seq_length):
        ...
        # class token
        self.class_token = nn.Parameter(torch.rand(size=(16,1,self.d_model))).to(device)
    def forward(self,x):
        #concat class_token (1,1,64) and x (x, 30,64)
        position = torch.LongTensor([[x for x in range(0, 17)]]).to(device)
        # print(x.shape, self.class_token.shape)
        x = torch.cat((self.class_token, x),dim=1)  # x.shape = (31,64)
        # print(f'x.shape = {x.shape}, position:{self.position_embedding(position).shape}')
        #实例化positon_encoding
...
class ActionTransformer(nn.Module):
    def __init__(self, d_model, d_ff, seq_length, n_head, dropout, keypoints, channels, encoder_layer, mask=None):
        ...
        self.ff1 = nn.Linear(in_features=self.d_model, out_features=12)
...
if __name__ == '__main__':
    model = ActionTransformer(d_model=192,
                              ...
                              keypoints=22,
                              channels=7,
                              encoder_layer=6).to(device)
    print(f'Number of parameters: {sum(p.numel() for p in model.parameters())}')
    summary(model, input_size=(16,16,22,7),device=device)